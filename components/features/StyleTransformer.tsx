...
const STYLES: Style[] = [
  ...
  { id: "classical", label: "古文風", prompt: "平安〜江戸時代の古典的な日本語の文体" },
  {
    ...
    label: "おじさん構文",
    prompt:
      "いわゆる「おじさん構文」に変換してください。以下のルールに従うこと。①原文に名前が含まれる場合のみカタカナのニックネームで呼ぶ。名前がない場合は呼びかけを追加しない②文中にガラケー時代の絵文字を3〜5個程度使う（😁💓😵🍺❗など）③古い顔文字を1〜2個使う（(^^♪ f^_^; (-_-;) など）④昭和〜平成初期のスラングは1個だけさりげなく混ぜる（「ナーンチャッテ」「トホホ」など）⑤さりげなく馴れ馴れしく、距離を縮めようとするニュアンスを含める⑥一文を短くして改行を多めにする。出力はそのままLINEで送れる文章にすること。",
  },
  {
    id: "dog",
    label: "犬向け",
...
function PanelHeader({
  ...
  label,
  right,
}: { num: string; label: string; right?: React.ReactNode }) {
  return (
    <div
      style={{
        padding: "0 1.5rem",
    ...
    >
      ...
      >
        <span style={{ color: C.inkMid, marginRight: "0.5rem", fontWeight: 400 }}>{num}</span>
        {label}
      </span>
      {right}
    </div>
  ...
  return (
    <div
      style={{
        ...
        height: isMobile ? "2rem" : undefined,
        flexShrink: 0,
        color: C.accent,
        fontSize: "0.75rem",
        pointerEvents: "none",
...
function StylePanel({
  ...
  return (
    ...
    >
      <PanelHeader num="02" label="文体を選ぶ" />
      <div
        style={{
          flex: isMobile ? undefined : 1,
  ...
  return (
    <span
      style={{
        ...
        borderRadius: "9999px",
        border: `1px solid ${style.highlight ? C.highlight : C.accent}`,
        color: style.highlight ? C.highlight : C.accent,
...
function OutputPanel({
  ...
  outputText: string;
  isLoading: boolean;
  activeStyleObj: Style | undefined;
  hasOutput: boolean;
  copied: boolean;
  onCopy: () => void;
  outputRef: React.RefObject<HTMLDivElement | null>;
      ...
      <PanelHeader
        ...
        right={
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            {activeStyleObj && <StyleBadge style={activeStyleObj} />}
            {hasOutput && !isLoading && <CopyButton copied={copied} onCopy={onCopy} />}
          </div>
        }
      />
      <OutputBody
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <InputPanel isMobile={isMobile} inputText={inputText} onInputChange={setInputText} />
